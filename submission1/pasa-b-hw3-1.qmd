...
```
###### 2.
Average Price per Pack, 1970-2018 (in 2012 dolalrs)
```{python}
...
###### 3.
Top 5 states with highest increases in cigarette prices:
```{python}
...
###### 4.
Top 5 states with lowes increases in cigarette prices:
```{python}
price_change = df.groupby('state')['cost_per_pack'].agg(['first', 'last'])
price_change['increase'] = price_change['last'] - price_change['first']
top5_states = price_change.sort_values(by='increase', ascending=False).head(5).index.tolist()
...
###### 5. 
Comparison of trens between these 10 states:
```{python}
