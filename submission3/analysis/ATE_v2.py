...
etable = pf.etable(
    [ols_70_90, iv_70_90],
    omit_stats="all",
    title=["OLS: log_sales ~ log_price", "IV: log_sales ~ log_price (instrumented)"],
    subheaders=["No instrument", "Instrument: total_tax"]
)

print(etable)
# -- Q 8 --
stage1 = pf.feols('log_price ~ log_total_tax', data=df_70_90)
...
reduced_reg = pf.feols('log_sales ~ log_total_tax', data=df_70_90)
print(f"Reduced Form: {reduced_reg.summary()}")
...
pf.etable(
    [ols_70_90, iv_70_90, ols_91_15, iv_91_15],
    omit_stats="all",
    title=["OLS: log_sales ~ log_price", "IV: log_sales ~ log_price (instrumented)"],
    subheaders=["No instrument", "Instrument: total_tax"]
)